...
// Initial cart value update (can remain global as it doesn't depend on product data)
...
async function loadProducts() {
  try {
    ...
    if (result.success) {
      products = result.data // Populate the global products array
      return result.data
    } else {
...
const showCartProduct = () => {
  // Clear existing items to prevent duplicates on re-render
  cartelement.innerHTML = ""

  const currentCartLS = getCartProduct() // Get the latest cart data from local storage

  const filteredProducts = products.filter((currProduct) => {
    return currentCartLS.some((currEle) => String(currProduct.productId) == String(currEle.productId))
  })

  filteredProducts.forEach((currProduct) => {
    const {
      _id,
    ...
    productClone.querySelector(".remove-btn").addEventListener("click", () => removeProductCart(productId)) // Use productId
...
async function onAppLoad() {
  ...
  showCartProduct() // Now display the cart products
  showATCT() // Show cart total after products are loaded
  showHam() // Show hamburger menu
}

document.addEventListener("DOMContentLoaded", onAppLoad)

// The rest of your functions (addProduct, handleContactFormSubmit, showPopUp, etc.) remain unchanged.
